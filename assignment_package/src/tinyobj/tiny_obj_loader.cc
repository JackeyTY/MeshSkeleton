...
std::string LoadObj(std::vector<shape_t> &shapes,
                    std::vector<material_t> &materials, std::vector<std::vector<int>> &faces,// [output]
    ...
    return LoadObj(shapes, materials, faces, vertices, ifs, matFileReader);
}
...
std::string LoadObj(std::vector<shape_t> &shapes,
    ...
    for(const std::vector<vertex_index> &f: faceGroup){
        std::vector<int> face;
        for(const vertex_index &v: f){
            face.push_back(v.v_idx);
        }
        faces.push_back(face);
    }

    for(const float &vx: v){
